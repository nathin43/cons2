...
if (-not (Test-Path "frontend\.env.local")) {
    Write-Host "[WARNING] frontend\.env.local not found. Creating..." -ForegroundColor Yellow
    $envContent = @"
...
Write-Host "[PID] Backend PID: $($backendProcess.Id)" -ForegroundColor Gray
...
Write-Host "[PID] Frontend PID: $($frontendProcess.Id)" -ForegroundColor Gray
Write-Host ""
...
try {
    while ($scriptRunning) {
        ...
        if ($backendProcess.HasExited) {
            Write-Host ""
            Write-Host "[ERROR] Backend process has stopped!" -ForegroundColor Red
        ...
        if ($frontendProcess.HasExited) {
            Write-Host ""
            Write-Host "[WARNING] Frontend process has stopped!" -ForegroundColor Yellow
